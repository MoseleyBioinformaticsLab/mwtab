...
for i, (name, group) in enumerate(groups):
    ...
    for name, final_group in final_groups:
        has_same_mz = False
        for column_name in mz_column_names:
            duplicated = final_group.loc[:, column_name].duplicated(keep=False)
            if (duplicated & ~final_group.loc[:, column_name].isna()).all():
                has_same_mz = True
                break
        has_same_rt = False
        for column_name in rt_column_names:
            duplicated = final_group.loc[:, column_name].duplicated(keep=False)
            if (duplicated & ~final_group.loc[:, column_name].isna()).all():
                has_same_rt = True
                break
        if mz_column_names and rt_column_names:
            if not has_same_mz or not has_same_rt:
                intermediate_df.loc[final_group.index, 'duplicate_group_attributes'] = 'Ambiguous'
        elif mz_column_names and not has_same_mz:
            intermediate_df.loc[final_group.index, 'duplicate_group_attributes'] = 'Ambiguous'
        elif rt_column_names and not has_same_rt:
            intermediate_df.loc[final_group.index, 'duplicate_group_attributes'] = 'Ambiguous'
