...
from . import metabolites_regexes, repair_shifted_rows

make_list_regex = metabolites_regexes.make_list_regex
ColumnFinder = repair_shifted_rows.ColumnFinder
NameMatcher = repair_shifted_rows.NameMatcher
ValueMatcher = repair_shifted_rows.ValueMatcher
...
# lines = []
# for key, attributes in column_matching_attributes.items():
#     attributes = {key2:value if 'values' not in key2 else '' for key2, value in attributes.items() if value and key2 not in ['regex', 'names', 'example_values']}
#     name_attributes = {key2:value for key2, value in attributes.items() if key2 not in ['values_regex', 'type', 'values_inverse_regex']}
#     values_attributes = {key2:value for key2, value in attributes.items() if key2 in ['values_regex', 'type', 'values_inverse_regex']}
    
#     init_string1 = f'ColumnFinder("{key}",'
#     init_string2 = f'NameMatcher('
#     name_space_string = ' '*(len('ColumnFinder(') + len(init_string2))
#     name_attribute_string = f'\n{name_space_string}'.join([f'{key2} = {value},' for key2, value in name_attributes.items()])
#     name_attribute_string += '),'
    
#     init_string3 = f'ValueMatcher('
#     value_space_string = ''*(len('ColumnFinder(') + len(init_string3))
#     value_attribute_string = f'\n{value_space_string}'.join([f'{key2} = {value},' for key2, value in values_attributes.items()])
#     value_attribute_string += ')'
    
#     final_string = init_string1 + '\n' + ' '*len('ColumnFinder(') + init_string2 + name_attribute_string + \
#                    '\n' + ' '*len('ColumnFinder(') + init_string3 + value_attribute_string + \
#                    '),'    
