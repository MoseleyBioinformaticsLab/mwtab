...
def test_errors():
    ...
    with pytest.raises(ValueError, match = r"^LINE WITH ERROR:.*") as exc_info:
    ...
    assert err.__cause__.args[0] == "Expected exactly 1 ':' in the factor key value pair, 'Disease Status::Cases'"
    ...
    assert hasattr(err, '__cause__')
    assert isinstance(err.__cause__, ValueError)
    assert err.__cause__.args[0] == "Expected exactly 1 '=' in the additional data key value pair, 'key1==value1'"
